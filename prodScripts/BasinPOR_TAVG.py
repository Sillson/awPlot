...
import warnings
import time
from lib.awdbToolsJson import dataUrl, trimToOct1, padMissingData, getBasinSites
...
def reject_outliers(data, m=3):
    with warnings.catch_warnings():
        ...
        mask = abs(data - np.nanmean(data)) < m * np.nanstd(data)
        data[:] = [x if mask[i] else np.nan for i, x in enumerate(data)]
    return data
...
def updtChart(basinName, basinSites):
    ...
    if validTrip:
        ...
        layout = go.Layout(
            ...
            yaxis=dict(title=r'Avg. Daily Temperature (&#176;F)',
                       hoverformat=".1f", tickformat="0f"),
            xaxis=dict(
                range=sliderDates,
                tickformat="%b %e",
                rangeselector=dict(
