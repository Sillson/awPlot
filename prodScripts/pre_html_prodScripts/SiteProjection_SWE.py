...
import math
import time
from lib.awdbToolsJson import padMissingData, ordinal, get_last_non_zero_index, createSWEProjTrace
...
def updtChart(site_triplet, siteName):
    print('Working on WTEQ Projection Chart for ' + siteName)
    statsData = []
    minData = []
    maxData = []
    meanData = []
    ...
    if validTrip:   
        ...
        for triplet in validTrip: 
            dataPath = path.join(master_dir,'prodScripts','data','norms',
                                 sensor, triplet.replace(':','_') + '.json')       
    ...
    data = []
    for triplet in validTrip: 
        dataPath = path.join(master_dir,'prodScripts','data',
                             sensor,triplet.replace(':','_') + '.json')
        with open(dataPath,'r') as j:
            jTemp = json.load(j)
        data.append(jTemp)
    ...
    layout = go.Layout(
        ...
        showlegend = True,
        title='Snow Water Equivalent Projections at<br>' + siteName,
...
if __name__ == '__main__':
    ...
    for state in states:
        sensor = r"WTEQ"
        dataPath = path.join(master_dir, 'prodScripts', 'data', 'metaData', sensor,
                             'metaData' + '.json')       
        with open(dataPath,'r') as j:
            meta = json.load(j)
        
        meta[:] = [x for x in meta if x['stationTriplet'].split(':')[1] in state]
