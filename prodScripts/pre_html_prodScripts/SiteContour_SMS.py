...
import time
from lib.awdbToolsJson import getSaturation
...
def updtChart(site_meta):
    ...
    if meta:
        for validSite in meta:
            ...
            dataPrec = []
            dataPath = path.join(master_dir,'prodScripts','data',
                             r'PREC',triplet.replace(':','_') + '.json')
            try:
                with open(dataPath,'r') as j:
                    jTemp = json.load(j)
                dataPrec.append(jTemp)
            except:
                dataPrec = [{"values": []}]
#might need to deal with mismatched start date between SMS and PREC
            dataSWE = []
            dataPath = path.join(master_dir,'prodScripts','data',
                             r'WTEQ',triplet.replace(':','_') + '.json')
            try:
                with open(dataPath,'r') as j:
                    jTemp = json.load(j)
                dataSWE.append(jTemp)
            except:
                dataSWE = [{"values": []}]
            maxPrecRng = 5
